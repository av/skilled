  ...
  return (
    <form className="settings" onSubmit={e => { e.preventDefault(); setSaving(true); void onSave(draft).finally(() => setSaving(false)); }}>
      <section className="panel">
        ...
        <label className="row"><span>Auto-refresh every</span>
          <select value={draft.refreshIntervalSecs} onChange={e => set("refreshIntervalSecs", Number(e.target.value))}>
            <option value={0}>never (manual / file watch only)</option><option value={30}>30 seconds</option><option value={60}>1 minute</option><option value={300}>5 minutes</option><option value={900}>15 minutes</option><option value={3600}>1 hour</option>
          </select>
        </label>
        ...
        {PATH_KEYS.map(p => {
          ...
          return (
            <label key={p.key} className="row path">
              <span>{p.label}<small className="muted">{p.hint}</small></span>
