...
/** Same seven sections, order, glyphs, tones and thresholds as `buildAuditLines()` in the TUI and `cmdAudit` in the CLI. */
export function Audit({ audit, onOpenSkill }: Props) {
  ...
  return (
    <section className="panel audit" aria-label="Skill audit">
      <h2><span className="glyph">▧</span> skill audit <span className="muted">{total} findings</span></h2>
      ...
      <div className="audit-grid">
        ...
        <Section icon="▲" tone="success" title="RISING" sub="50%+ growth last 4w" items={audit.rising.map(r => ({ skill: r.skill.skill, detail: `now: ${r.recentCount}  was: ${r.priorCount}  ↑${r.pct}%` }))} onOpen={onOpenSkill} />
        <Section icon="▼" tone="danger" title="DECLINING" sub="50%+ drop last 4w" items={audit.declining.map(d => ({ skill: d.skill.skill, detail: `now: ${d.recentCount}  was: ${d.priorCount}  ↓${d.pct}%` }))} onOpen={onOpenSkill} />
        <Section icon="⚠" tone="warn" title="STALE" sub="unused 28+ days" items={audit.stale.map(s => ({ skill: s.skill, detail: `last: ${timeAgo(s.lastUsed)}  ${s.count} calls` }))} onOpen={onOpenSkill} />
...
function Section({ icon, tone, title, sub, items, onOpen }: { icon: string; tone: string; title: string; sub: string; items: { skill: string; detail: string }[]; onOpen: (s: string) => void }) {
  return (
    <section className={`audit-section ${tone}`} aria-label={title}>
      ...
      {items.length === 0 ? <div className="none">none</div> : (
        <ul>{items.map(i => <li key={i.skill}><button className="link skill" onClick={() => onOpen(i.skill)} title="Open detail">{i.skill}</button><span className="detail">{i.detail}</span></li>)}</ul>
      )}
    </section>
  );
}
