    ...
    <div className="providers">
      <section className="panel index-status" aria-label="Index status">
        ...
        <div className="kv wide">
          ...
          <div><span className="k">last indexed</span><span className="v" title={fmtDateTime(indexed)}>{fmtDateTime(indexed)} {snapshot.reindexed ? "(re-parsed this refresh)" : "(fresh index reused)"}</span></div>
          ...
          <div><span className="k">index version</span><span className="v">{snapshot.indexVersion ?? "—"} · app {info?.version ?? "?"} · {info?.platform}</span></div>
        ...
        <p className="muted small">Local files only. Skilled never sends data anywhere; there is no network access in this app.</p>
      </section>
          ...
          <tbody>
            {snapshot.providers.map(p => (
              <tr key={p.slug} className={p.available ? "" : "dim"}>
                ...
                <td><button className="link" onClick={() => void reveal(p.path)} disabled={!p.available}>reveal</button></td>
              </tr>
            ))}
          </tbody>
        </table>
