  ...
  return (
    <div className="columns detail-layout">
      ...
      <section className="panel detail" aria-label={`Detail for ${d.skill}`}>
        <header>
          <h2><span className="glyph">◆</span> {d.skill}</h2>
        ...
        <div className="kv">
          ...
          <div><span className="k">first</span><span className="v" title={fmtDateTime(d.firstUsed)}>{timeAgo(d.firstUsed)} ago</span></div>
          <div><span className="k">last</span><span className="v" title={fmtDateTime(d.lastUsed)}>{timeAgo(d.lastUsed)} ago</span></div>
        </div>
        <h3><span className="glyph">▸</span> weekly trend <span className="muted">16 weeks</span></h3>
        <div className="sparkline" role="img" aria-label={`Weekly usage: ${d.weeklyUsage.join(", ")}`}>
          {d.weeklyUsage.map((v, i) => <span key={i} className="bar" style={{ height: `${(v / maxWeek) * 100}%`, animationDelay: `${i * 30}ms` }} title={`${15 - i} weeks ago: ${v}`} />)}
        </div>
        <div className="spark-axis"><span>-16w</span><span>now</span></div>
        <h3><span className="glyph">▪</span> by project</h3>
        ...
        <h3><span className="glyph">≡</span> calls <span className="muted">{calls.length}</span></h3>
